...
MapView.prototype._createLabelA = function  (driver) {
    ...
    this._lbSetListenerEvent();
}
MapView.prototype._lbSetListenerEvent = function () {
    var numReg = /\d+/;
    $(".mapLab").hover(function () {
        var num = parseInt(this.id.match(numReg));
        $("#ptr" + num).addClass('hover');
        $(".taP").animate({scrollTop: $("#TbBody tr").height() * num});
    }, function () {
        var num = parseInt(this.id.match(numReg));
...
TableView.prototype.fillTable = function (sIndex, eIndex) {
    ...
    for (i = sIndex,j = 0; i < eIndex; i++,j++) {
        ...
        dom +='<tr id="'+ trId + j +'" class="'+ cls +'"><td class="tbId">'
            + data.id + '<a  class="tbUrl" target="_blank" href="http://192.168.0.175:8008/baiduImg.jsp?'+ data.url +'">&nbsp;</a></td><td class="tbDirection">'
...
function bindDomEvent() {
    ...
    Dom.attachToMap.bind("click", function () {
        ...
        loadingPlugis.start();
        setTimeout(function () {Kbfunction.analyze();},0);
    });
