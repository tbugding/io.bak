...
(function () {
    ...
    baidu.dom.g = function (id) {
        if ("string" == typeof id || id instanceof String)return Tdocument.getElementById(id); else if (id && id.nodeName && (id.nodeType == 1 || id.nodeType == 9))return id;
        return null
    };
    ...
    baidu.dom.setAttr = function (element, key, value) {
        element = baidu.dom.g(element);
        if ("style" == key)element.style.cssText = value; else {
    ...
    baidu.dom.insertHTML = function (element, position, html) {
        element = baidu.dom.g(element);
        var range, begin;
        if (element.insertAdjacentHTML)element.insertAdjacentHTML(position, html); else {
    ...
    baidu.dom.addClass = function (element, className) {
        element = baidu.dom.g(element);
        var classArray = className.split(/\s+/), result = element.className, classMatch = " " + result + " ", i = 0, l = classArray[Tlength];
    ...
    baidu.dom.getStyle = function (element, key) {
        ...
        if (!value) {
            var fixer = dom._styleFixer[key], style = element.currentStyle || (baidu.browser.ie ? element[Tstyle] : getComputedStyle(element, null));
    ...
    baidu.dom.getPosition = function (element) {
        ...
        if (element.getBoundingClientRect) {
            ...
            var htmlDom = doc.body, htmlBorderLeftWidth = TparseInt(getStyle(htmlDom, "borderLeftWidth")), htmlBorderTopWidth = TparseInt(getStyle(htmlDom, "borderTopWidth"));
    ...
    baidu.lang.Class[Tprototype][TaddEventListener] = function (type, handler, key) {
        ...
        if (typeof key == "string" && key)if (/[^\w\-]/.test(key))throw"nonstandard key:" + key; else {
    ...
    baidu.lang.Class[Tprototype][TremoveEventListener] = function (type, handler) {
        if (baidu.lang.isFunction(handler))handler = handler.hashCode; else if (!baidu.lang.isString(handler))return;
        !this.__listeners && (this.__listeners = {});
        type.indexOf("on") != 0 && (type = "on" + type);
    ...
    baidu.lang.Event[Tprototype].inherit = function (e) {
        ...
        if (e.touches) {
            me.touches = [];
            for (var i = 0; i < e.touches[Tlength]; i++)me.touches.push({clientX: e.touches[i].clientX, clientY: e.touches[i].clientY, screenX: e.touches[i].screenX, screenY: e.touches[i].screenY, pageX: e.touches[i].pageX, pageY: e.touches[i].pageY, target: e.touches[i].target, identifier: e.touches[i].identifier})
        }
        if (e.changedTouches) {
            me.changedTouches = [];
            for (var i = 0; i < e.changedTouches[Tlength]; i++)me.changedTouches.push({clientX: e.changedTouches[i].clientX, clientY: e.changedTouches[i].clientY, screenX: e.changedTouches[i].screenX, screenY: e.changedTouches[i].screenY, pageX: e.changedTouches[i].pageX, pageY: e.changedTouches[i].pageY, target: e.changedTouches[i].target, identifier: e.changedTouches[i].identifier})
        }
        if (e.targetTouches) {
            me.targetTouches = [];
            for (var i = 0; i < e.targetTouches[Tlength]; i++)me.targetTouches.push({clientX: e.targetTouches[i].clientX, clientY: e.targetTouches[i].clientY, screenX: e.targetTouches[i].screenX, screenY: e.targetTouches[i].screenY, pageX: e.targetTouches[i].pageX, pageY: e.targetTouches[i].pageY, target: e.targetTouches[i].target, identifier: e.targetTouches[i].identifier})
        }
    ...
    baidu.on = baidu.event.on = function (el, type, handler) {
        ...
        if (el[TaddEventListener])el[TaddEventListener](type, handler, false); else if (el.attachEvent)el.attachEvent("on" + type, handler);
        return el
    };
    baidu.un = baidu.event.un = function (el, type, handler) {
        if (!(el = baidu.g(el)))return el;
        type = type.replace(/^on/, "");
        if (el[TremoveEventListener])el[TremoveEventListener](type, handler, false); else if (el.detachEvent)el.detachEvent("on" + type, handler);
        return el
    };
    ...
    baidu.enableTapEvent = function () {
        ...
        Tap[Tprototype] = {hasTouch: "ontouchstart"in window || "createTouch"in document, start: function (e) {
            stopBubble(e);
            this.moved = false;
            ...
            stopAndPrevent(e);
            var x = this.hasTouch ? e.touches[0].clientX : e.clientX, y = this.hasTouch ? e.touches[0].clientY : e.clientY;
    ...
    function Map(element, opts) {
        ...
        var opts = opts || {};
        me.config = {clickInterval: 200, enableDragging: true, enableKeyboard: false, enableDblclickZoom: true, enableContinuousZoom: false, enableWheelZoom: false, enableMouseDown: true, enablePinchToZoom: true, enableAutoResize: true, fps: 25, zoomerDuration: 240, actionDuration: 450, defaultCursor: MapConfig.defaultCursor, draggingCursor: MapConfig.draggingCursor, isOverviewMap: !!opts.isOverviewMap, minZoom: opts["minZoom"] || 1, maxZoom: opts["maxZoom"] || 18, mapType: opts["mapType"] || BMAP_NORMAL_MAP, restrictBounds: false, enableInertialDragging: false, drawMargin: 500, enableHighResolution: !(opts["enableHighResolution"] === false), enableMapClick: !(opts["enableMapClick"] === false)};
        ...
        me._checkZoom();
        me.temp = {operating: false, arrow: 0, lastDomMoveTime: 0, lastLoadTileTime: 0, lastMovingTime: 0, canKeyboard: false, registerIndex: -1, curSpots: []};
    ...
    baidu[Textend](Map[Tprototype], {render: function () {
        ...
        s.zIndex = "0";
        s.top = s.left = "0px";
        var mask = create("div", {"class": "BMap_mask"});
        var ms = mask.style;
        ...
        var cPoint;
        if (point)cPoint = point; else if (this.getInfoWindow())cPoint = this.getInfoWindow().getPosition();
        if (cPoint) {
        ...
        if (Point.isInRange(center))this.centerPoint = new Point(center["lng"], center["lat"]); else this.centerPoint = this.projection.mercatorToLngLat(this.mercatorCenter, this.currentCity)
        ...
        var me = this;
        if (center instanceof Point)me.panTo(center, {"noAnimation": true}); else if (isString(center)) {
            ...
            local.search(center, {log: "center"})
        }
        ...
        if (this.customSize && this.customSize instanceof Size)return new Size(this.customSize[Twidth], this.customSize[Theight]); else return new Size(this.container.clientWidth, this.container.clientHeight)
        ...
        return{zoom: targetZoom, exceeded: exceeded}
        ...
        var opts = arguments[0] || {}, margins = opts["margins"] || [0, 0, 0, 0], zoom = opts["zoom"] || null, pt1 = this.pixelToPoint({x: margins[3], y: this[Theight] - margins[2]}, zoom), pt2 = this.pixelToPoint({x: this[Twidth] - margins[1], y: margins[0]}, zoom);
        return new Bounds(pt1, pt2)
    }, isLoaded: function () {
        return!!this.loaded
    }, _getBestLevel: function (bounds, opts) {
        var mapType = this.getMapType();
        var margins = opts["margins"] || [10, 10, 10, 10], zoomFactor = opts["zoomFactor"] || 0, ew = margins[1] + margins[3], eh = margins[0] + margins[2], minLevel = mapType.getMinZoom(), maxLevel = mapType.getMaxZoom();
        ...
        var curViewport = {"center": this.getCenter(), "zoom": this.getZoom()};
        ...
        if (opts["margins"]) {
            var margins = opts["margins"], wBias = (margins[1] - margins[3]) / 2, hBias = (margins[0] - margins[2]) / 2, zoomUnit = this.getMapType().getZoomUnits(zoom);
            if (opts["offset"]) {
        ...
        var viewport;
        if (view && view["center"])viewport = view; else viewport = this.getViewport(view, opts);
        opts = opts || {};
        var delay = opts["delay"] || 200;
        if (viewport.zoom == this.zoomLevel && opts["enableAnimation"] != false) {
            var me = this;
            setTimeout(function () {
                me.panTo(viewport["center"], {"duration": 210})
            }, delay)
        ...
        if (!tag)tag = "custom"; else if (!this._hotspots[tag])this._hotspots[tag] = {};
        hotspot["tag"] = tag;
        ...
        this._checkZoom();
        var config = this.config;
        if (this.zoomLevel < config.minZoom)this.setZoom(config.minZoom); else if (this.zoomLevel > config.maxZoom)this.setZoom(config.maxZoom);
    ...
    function _addStat(code, opts) {
        ...
        if (_addStat._sending)_addStat._reqQueue.push({src: "product=jsapi&v=" + BMap.version + "&t=" + ts + "&code=" + code + extq}); else sendStat({src: "product=jsapi&v=" + BMap.version + "&t=" + ts + "&code=" + code + extq});
        if (!_addStat._binded) {
    ...
    function Animation(opts) {
        var defaultOptions = {duration: 1E3, fps: 30, delay: 0, transition: Transitions.linear, onStop: function () {
        }};
        this._anis = [];
    ...
    Animation[Tprototype].setFinishCallback = function (callback) {
        if (this._anis[Tlength] > 0)this._anis[this._anis[Tlength] - 1]._opts.finish = callback; else this._opts.finish = callback
    };
    var Transitions = {linear: function (t) {
        return t
    }, reverse: function (t) {
        return 1 - t
        ...
        if (t < 0.5)return t * t * 2; else return-2 * (t - 2) * t - 1;
        return
    }, easeInOutCubic: function (t) {
        if (t < 0.5)return TMath.pow(t, 3) * 4; else return TMath.pow(t - 1, 3) * 4 + 1
    ...
    var MapConfig = {imgPath: "http://api.map.baidu.com/images/", cityNames: {"\u5317\u4eac": {pinyin: "bj", pt: new Point(116.403874, 39.914889)}, "\u4e0a\u6d77": {pinyin: "sh", pt: new Point(121.487899, 31.249162)}, "\u6df1\u5733": {pinyin: "sz", pt: new Point(114.025974, 22.546054)}, "\u5e7f\u5dde": {pinyin: "gz", pt: new Point(113.30765, 23.120049)}}, fontFamily: "arial,sans-serif"};
    if (baidu.browser.firefox) {
        baidu[Textend](MapConfig, {distCursor: "url(" + MapConfig.imgPath + "ruler.cur),crosshair", defaultCursor: "-moz-grab", draggingCursor: "-moz-grabbing"});
        if (baidu.platform.isWindows)MapConfig.fontFamily = "arial,simsun,sans-serif"
    } else if (baidu.browser.chrome || baidu.browser.safari)baidu[Textend](MapConfig, {distCursor: "url(" + MapConfig.imgPath + "ruler.cur) 2 6,crosshair", defaultCursor: "url(" + MapConfig.imgPath + "openhand.cur) 8 8,default", draggingCursor: "url(" + MapConfig.imgPath + "closedhand.cur) 8 8,move"}); else baidu[Textend](MapConfig, {distCursor: "url(" + MapConfig.imgPath + "ruler.cur),crosshair", defaultCursor: "url(" + MapConfig.imgPath + "openhand.cur),default", draggingCursor: "url(" + MapConfig.imgPath + "closedhand.cur),move"});
    function Copyright(id, bounds, content) {
        this.id = id;
        this.bounds = bounds;
        this.content = content
    };
    var Operation = {undo: 1, redo: 2, zoom: 4, drag: 8, move: 16, mousewheel: 32, toolbarOperation: 64, stdMapCtrlDrag: 128, dblclick: 256};
    ...
    function setUnSelectable(obj) {
        if (baidu.browser.ie > 0)obj.unselectable = "on"; else obj.style.MozUserSelect = "none"
    }
    ...
    function getCurrentStyle(el) {
        if (el.currentStyle)return el.currentStyle; else if (el.ownerDocument && el.ownerDocument.defaultView)return el.ownerDocument.defaultView.getComputedStyle(el, null)
    }
    ...
    var DataMgr = {request: function (url) {
        var script = create("script", {"src": url, "type": "text/javascript", "charset": "utf-8"});
        if (script[TaddEventListener])script[TaddEventListener]("load", function (e) {
            var t = e.target;
            t[TparentNode][TremoveChild](t)
        }, false); else if (script.attachEvent)script.attachEvent("onreadystatechange", function (e) {
    ...
    var ModuleVersion = {"map": "20140320104737", "tile": "20140320104737", "marker": "20140320104737", "markeranimation": "20140320104737", "poly": "20140320104737", "draw": "20140320104737", "drawbysvg": "20140320104737", "drawbyvml": "20140320104737", "drawbycanvas": "20140320104737", "infowindow": "20140320104737", "oppc": "20140320104737", "opmb": "20140320104737", "menu": "20140320104737", "control": "20140320104737", "navictrl": "20140320104737", "geoctrl": "20140320104737", "copyrightctrl": "20140320104737", "scommon": "20140320104737", "local": "20140320104737", "route": "20140320104737", "othersearch": "20140320104737", "buslinesearch": "20140320104737", "hotspot": "20140320104737", "autocomplete": "20140320104737", "coordtrans": "20140320104737", "coordtransutils": "20140320104737", "mapclick": "20140320104737"};
    baidu.storage = function () {
        var _prefix = "BMap_", ls = window.localStorage, _isSupportLocalStorage = "localStorage"in window && ls !== null && ls !== undefined;
        ...
        function _get(moduleName) {
            if (_isSupportLocalStorage && _check(moduleName))return ls.getItem(_getRealyModuleName(moduleName)); else return false
        }

        return{isSupportLocalStorage: _isSupportLocalStorage, set: _set, get: _get, check: _check}
    }();
    function ModuleMgr() {
    }

    baidu.object[Textend](ModuleMgr, {Request: {INITIAL: -1, WAITING: 0, COMPLETED: 1}, getDependency: function () {
        var drawType = "drawbysvg";
        if (isSvg())drawType = "drawbysvg"; else if (isVml())drawType = "drawbyvml"; else if (isCanvas())drawType = "drawbycanvas";
        return{"control": [], "marker": [], "poly": ["marker", drawType], "drawbysvg": ["draw"], "drawbyvml": ["draw"], "drawbycanvas": ["draw"], "infowindow": ["marker"], "menu": [], "oppc": [], "opmb": [], "scommon": [], "local": ["scommon"], "route": ["scommon"], "othersearch": ["scommon"], "autocomplete": ["scommon"], "mapclick": ["scommon"], "buslinesearch": ["route"], "hotspot": [], "coordtransutils": ["coordtrans"]}
    }, preLoaded: {}, Config: {_baseUrl: "http://api.map.baidu.com/getmodules?v=1.4", _timeout: 5E3}, delayFlag: false, Module: {_modules: {}, _arrMdls: [], _tempMdls: []}, load: function (moduleName, callback, bExeLoadedCbk) {
                ...
                if (me.delayFlag == false) {
                    ...
                    setTimeout(function () {
                        ...
                        for (var i = 0, l = me.Module._arrMdls[Tlength]; i < l; i++) {
                            ...
                            me.Module._tempMdls.push({mdlName: mdlName, mdlCode: mdlCode})
                        }
                        me.delayFlag = false;
                        me.Module._arrMdls[Tlength] = 0;
                        if (mdlsFromServer[Tlength] == 0)me.evalAllCode(); else {
        ...
        for (var i = 0, l = tempMdl[Tlength]; i < l; i++)if (tempMdl[i].mdlCode == "")if (tempMdl[i].mdlName == mdlName)tempMdl[i].mdlCode = mdlCode; else return;
        this.evalAllCode()
    ...
    Hotspot.guid = 0;
    baidu[Textend](Hotspot[Tprototype], {initialize: function (map) {
    ...
    baidu[Textend](Control[Tprototype], {"initialize": function (map) {
        this._map = map;
        if (this._container) {
    ...
    function NavigationControl(opts) {
        Control.call(this);
        opts = opts || {};
        this._opts = {printable: false, showZoomInfo: opts["showZoomInfo"] || true, anchor: opts["anchor"], offset: opts["offset"], type: opts["type"]};
    ...
    baidu[Textend](NavigationControl[Tprototype], {"initialize": function (map) {
        this._map = map;
        return this._container
    }, setType: function (type) {
        if (isNumber(type) && type >= BMAP_NAVIGATION_CONTROL_LARGE && type <= BMAP_NAVIGATION_CONTROL_ZOOM)this._opts.type = type; else this._opts.type = BMAP_NAVIGATION_CONTROL_LARGE
    }, getType: function () {
    ...
    function GeolocationControl(opts) {
        Control.call(this);
        opts = opts || {};
        this._opts = {anchor: opts["anchor"], offset: opts["offset"], showAddressBar: opts["showAddressBar"], enableAutoLocation: opts["enableAutoLocation"], locationIcon: opts["locationIcon"]};
    ...
    baidu[Textend](GeolocationControl[Tprototype], {"initialize": function (map) {
        this._map = map;
        return this._container
    }, _asyncLoadCode: function () {
        var me = this;
    ...
    function CopyrightControl(opts) {
        Control.call(this);
        opts = opts || {};
        this._opts = {printable: false, anchor: opts["anchor"], offset: opts["offset"]};
    ...
    baidu.object[Textend](CopyrightControl[Tprototype], {"initialize": function (map) {
        this._map = map;
        return this._container
    }, addCopyright: function (copyright) {
        if (!copyright || !isNumber(copyright["id"]) || isNaN(copyright["id"]))return;
        var ncpy = {"bounds": null, "content": ""};
        for (var i in copyright)ncpy[i] = copyright[i];
        var cp = this.getCopyright(copyright["id"]);
        if (cp)for (var o in ncpy)cp[o] = ncpy[o]; else this._copyrightCollection.push(ncpy)
    ...
    function OverviewMapControl(opts) {
        Control.call(this);
        opts = opts || {};
        this._opts = {printable: false, size: opts["size"] || new Size(150, 150), padding: 5, isOpen: opts["isOpen"] === true ? true : false, zoomInterval: 4, offset: opts["offset"], anchor: opts["anchor"]};
    ...
    baidu[Textend](OverviewMapControl[Tprototype], {"initialize": function (map) {
        this._map = map;
        return this._container
    }, setAnchor: function (anchor) {
    ...
    function ScaleControl(opts) {
        Control.call(this);
        opts = opts || {};
        this._opts = {printable: false, color: "black", unit: "metric", offset: opts["offset"]};
        this["defaultAnchor"] = BMAP_ANCHOR_BOTTOM_LEFT;
        this["defaultOffset"] = new Size(81, 18);
        this.setAnchor(opts["anchor"]);
        this._units = {"metric": {name: "metric", conv: 1, incon: 1E3, u1: "\u7c73", u2: "\u516c\u91cc"}, "us": {name: "us", conv: 3.2808, incon: 5280, u1: "\u82f1\u5c3a", u2: "\u82f1\u91cc"}};
    ...
    baidu.object[Textend](ScaleControl[Tprototype], {"initialize": function (map) {
        this._map = map;
        return this._container
    ...
    function MapTypeControl(opts) {
        ...
        this["defaultOffset"] = new Size(10, 10);
        this._opts = {printable: false, mapTypes: [BMAP_NORMAL_MAP, BMAP_SATELLITE_MAP, BMAP_HYBRID_MAP, BMAP_PERSPECTIVE_MAP], type: opts["type"] || BMAP_MAPTYPE_CONTROL_HORIZONTAL, offset: opts["offset"] || this["defaultOffset"], enableSwitch: true};
        this.setAnchor(opts["anchor"]);
    ...
    baidu.object[Textend](MapTypeControl[Tprototype], {"initialize": function (map) {
        this._map = map;
        return this._container
    }, _asyncLoadCode: function () {
        var me = this;
    ...
    baidu.object[Textend](ContextMenu[Tprototype], {initialize: function (map, overlay) {
        this._map = map;
        ...
        this._items.push({_type: "divider", _dIndex: this._dividers[Tlength]});
    ...
    baidu.object[Textend](MenuItem[Tprototype], {initialize: function (map, menu) {
        this._map = map;
    ...
    baidu.object[Textend](Bounds[Tprototype], {isEmpty: function () {
    ...
    var CoordTransUtils = {convertPtToPix3D: function (point, map, cbk) {
    ...
    baidu[Textend](MercatorProjection, {EARTHRADIUS: 6370996.81, MCBAND: [1.289059486E7, 8362377.87, 5591021, 3481989.83, 1678043.12, 0], LLBAND: [75, 60, 45, 30, 15, 0], MC2LL: [
    ...
    baidu[Textend](MercatorProjection[Tprototype], {lngLatToMercator: function (point) {
    ...
    baidu[Textend](Overlay[Tprototype], {_i: function (map) {
    ...
    baidu[Textend](OverlayInternal[Tprototype], {"initialize": function (map) {
        this.map = map;
    ...
    BMap.register(function (map) {
        ...
        map[TaddEventListener]("removeoverlay", function (e) {
            var overlay = e.target;
            if (overlay instanceof OverlayInternal)delete mgr._overlays[overlay.guid]; else for (var i = 0, l = mgr._customOverlays[Tlength]; i < l; i++)if (mgr._customOverlays[i] === overlay) {
    ...
    baidu[Textend](Graph[Tprototype], {"initialize": function (map) {
        this.map = map;
        return null
    }, "draw": function () {
        return;
    ...
    function Division(opts) {
        ...
        this.map = null;
        this.domElement = null;
        this._config = {width: 0, height: 0, offset: new Size(0, 0), opacity: 1, background: "transparent", lineStroke: 1, lineColor: "#000", lineStyle: "solid", point: null};
        this.setConfig(opts);
        this.point = this._config.point
    }

    baidu.lang.inherits(Division, OverlayInternal, "Division");
    baidu[Textend](Division[Tprototype], {_addDom: function () {
    ...
    function Icon(imageUrl, size, opts) {
        ...
        opts = opts || {};
        var _config = {anchor: opts["anchor"] || defaultAnchor, imageOffset: opts["imageOffset"] || new Size(0, 0)};
    ...
    baidu[Textend](Icon[Tprototype], {setImageUrl: function (url) {
        if (!url)return;
        this["imageUrl"] = url
    ...
    function InfoWindow(content, opts) {
        ...
        this.map = null;
        opts = opts || {};
        this._config = {width: opts["width"] || 0, height: opts["height"] || 0, maxWidth: opts["maxWidth"] || 600, offset: opts["offset"] || new Size(0, 0), title: opts["title"] || "", maxContent: opts["maxContent"] || "", enableMaximize: opts["enableMaximize"] || false, enableAutoPan: opts["enableAutoPan"] === false ? false : true, enableCloseOnClick: opts["enableCloseOnClick"] === false ? false : true, margin: [10, 10, 40, 10], collisions: [
            [10, 10],
            [10, 10],
            [10, 10],
            [10, 10]
    ...
    baidu[Textend](InfoWindow[Tprototype], {"setWidth": function (width) {
    ...
    Map[Tprototype].openInfoWindow = function (infoWin, point) {
        ...
        var t = this.temp;
        if (!t.marker) {
            var icon = new Icon(MapConfig.imgPath + "blank.gif", {"width": 1, "height": 1});
            t.marker = new Marker(point, {"icon": icon, "offset": new Size(0, 0), "clickable": false});
            t.marker._fromMap = 1
    ...
    function Label(content, opts) {
        ...
        this.domElement = null;
        opts = opts || {};
        this._config = {width: 0, offset: opts["offset"] || new Size(0, 0), styles: {backgroundColor: "#fff", border: "1px solid #f00", padding: "1px", whiteSpace: "nowrap", font: "12px " + MapConfig.fontFamily, zIndex: "80", MozUserSelect: "none"}, position: opts["position"] || null, enableMassClear: opts["enableMassClear"] === false ? false : true, clickable: true};
    ...
    baidu[Textend](Label[Tprototype], {getPosition: function () {
        ...
        this._marker = marker;
        if (marker)this.point = this._config[Tposition] = marker.getPosition(); else this.point = this._config[Tposition] = null
    }, getMarker: function () {
        return this._marker || null
    }});
    var BMAP_ANIMATION_DROP = 1, BMAP_ANIMATION_BOUNCE = 2;
    var DEFAULT_ICON = new Icon(MapConfig.imgPath + "marker_red_sprite.png", new Size(19, 25), {"anchor": new Size(10, 25), "infoWindowAnchor": new Size(10, 0)});
    var DEFAULT_SHADOW = new Icon(MapConfig.imgPath + "marker_red_sprite.png", new Size(20, 11), {"anchor": new Size(6, 11), "imageOffset": new Size(-19, -13)});
    ...
    function Marker(point, opts) {
        ...
        this._config = {offset: opts["offset"] || new Size(0, 0), icon: opts["icon"] || DEFAULT_ICON, shadow: DEFAULT_SHADOW, title: opts["title"] || "", label: null, baseZIndex: opts["baseZIndex"] || 0, clickable: true, zIndexFixed: false, isTop: false, enableMassClear: true, enableDragging: false, raiseOnDrag: opts["raiseOnDrag"] || false, restrictDraggingArea: false, draggingCursor: opts["draggingCursor "] || MapConfig.draggingCursor};
    ...
    baidu[Textend](Marker[Tprototype], {setIcon: function (icon) {
    ...
    function Polygon(points, opts) {
        ...
        if (opts.fillColor == "")this._config.fillColor = ""; else this._config.fillColor = opts["fillColor"] ? opts["fillColor"] : "#fff";
        this.setPath(points);
        var me = this;
        ModuleMgr.load("poly", function () {
            me._draw()
    ...
    baidu[Textend](Polygon[Tprototype], {setPath: function (points, flag) {
    ...
    baidu[Textend](Circle[Tprototype], {"initialize": function (map) {
        this.map = map;
        ...
        var points = [];
        var d = radius / 6378800, lat1 = TMath.PI / 180 * cy, lng1 = TMath.PI / 180 * cx;
        for (var i = 0; i < 360; i += 9) {
            var tc = TMath.PI / 180 * i, y = TMath.asin(TMath.sin(lat1) * TMath.cos(d) + TMath.cos(lat1) * TMath.sin(d) * TMath.cos(tc)), dlng = TMath.atan2(TMath.sin(tc) * TMath.sin(d) * TMath.cos(lat1), TMath.cos(d) - TMath.sin(lat1) * TMath.sin(y)), x = (lng1 - dlng + TMath.PI) % (2 * TMath.PI) - TMath.PI, point = new Point(x * (180 / TMath.PI), y * (180 / TMath.PI));
            points.push(point)
        }
    ...
    baidu[Textend](TileMgr[Tprototype], {initialize: function () {
        ...
        if (tile && tile.img) {
            ...
            if (tile.loaded)this._checkTilesLoaded(); else tile._addLoadCbk(function () {
                ...
                if (me.numLoading == 0) {
                    ...
                    if (isFirstTilesLoaded) {
                        if (TIME_LINE.startLoadScript && TIME_LINE.loadScriptEnd && TIME_LINE.loadedMap) {
                            ...
                            setTimeout(function () {
                                _addStat(5030, {"load_script_time": TIME_LINE.loadScriptEnd - TIME_LINE.startLoadScript, "load_tiles_time": currentTime - TIME_LINE.loadedMap, "map_width": size[Twidth], "map_height": size[Theight], "map_size": size[Twidth] * size[Theight]})
                            }, 1E4)
                        }
        ...
        if (this.map.getMapType() === BMAP_PERSPECTIVE_MAP)return"TILE-" + tilelayer.guid + "-" + this.map.cityCode + "-" + tileInfo[0] + "-" + tileInfo[1] + "-" + tileInfo[2]; else return"TILE-" + tilelayer.guid + "-" + tileInfo[0] + "-" + tileInfo[1] + "-" + tileInfo[2]
    ...
    function Tile(mgr, src, position, tileInfo, tilelayer) {
        ...
        this.img.onload = function (e) {
            ...
            if (fadeInTile)new Animation({fps: 20, duration: 200, render: function (s) {
    ...
    Tile[Tprototype]._load = function () {
        if (baidu.browser.ie > 0 && baidu.browser.ie <= 6 && this._transparentPng)this.img.src = MapConfig.imgPath + "blank.gif"; else this.img.src = this.src
    };
    ...
    baidu[Textend](TileLayer[Tprototype], {initialize: function (map, container) {
    ...
    TrafficLayer[Tprototype]["getTilesUrl"] = function (point, level) {
        var pDate = "";
        if (this._opts["predictDate"])pDate = "HistoryService?" + "day=" + (this._opts["predictDate"]["weekday"] - 1) + "&hour=" + this._opts["predictDate"]["hour"] + "&t=" + (new Date).getTime() + "&"; else {
            pDate = "TrafficTileService?" + "time=" + (new Date).getTime() + "&";
            if (!this._map.highResolutionEnabled())pDate += "label=web2D&v=016&"
        }
        var map = this._map, row = point.x, column = point.y, px = TMath.floor(row / 200), py = TMath.floor(column / 200), url = this._tileUrl + pDate + "level=" + level + "&x=" + row + "&y=" + column;
    ...
    function MapType(name, layer, opts) {
        ...
        opts = opts || {};
        this._opts = {tips: opts["tips"] || "", labelText: "", minZoom: opts["minZoom"] || 3, maxZoom: opts["maxZoom"] || 19, highMinZoom: opts["minZoom"] || 3, highMaxZoom: opts["maxZoom"] || 18, tileSize: 256, textColor: opts["textColor"] || "black", errorImageUrl: opts["errorImageUrl"] || "", projection: opts["projection"] || new MercatorProjection};
        if (this._layers[Tlength] >= 1)this._layers[0].baseLayer = true;
        baidu[Textend](this._opts, opts)
    }

    baidu[Textend](MapType[Tprototype], {getName: function () {
        return this._name
    }, getTips: function () {
    ...
    var BMAP_NORMAL_MAP = new MapType("\u5730\u56fe", normalMapTileLayer, {"tips": "\u663e\u793a\u666e\u901a\u5730\u56fe"});
    ...
    var BMAP_PERSPECTIVE_MAP = new MapType("\u4e09\u7ef4", PerspectiveTileLayer, {"tips": "\u663e\u793a\u4e09\u7ef4\u5730\u56fe", "minZoom": 15, "maxZoom": 20, "textColor": "white", "projection": new PerspectiveProjection});
    ...
    BMAP_PERSPECTIVE_MAP.getCityCode = function (city) {
        return{"bj": 2, "gz": 1, "sz": 14, "sh": 4}[city]
    };
    var satelliteTileLayer = new TileLayer({baseLayer: true});
    ...
    var BMAP_SATELLITE_MAP = new MapType("\u536b\u661f", satelliteTileLayer, {"tips": "\u663e\u793a\u536b\u661f\u5f71\u50cf", "minZoom": 1, "maxZoom": 19, "textColor": "white"});
    var streetLayer = new TileLayer({"transparentPng": true});
    ...
    var BMAP_HYBRID_MAP = new MapType("\u6df7\u5408", [satelliteTileLayer, streetLayer], {"tips": "\u663e\u793a\u5e26\u6709\u8857\u9053\u7684\u536b\u661f\u5f71\u50cf", "labelText": "\u8def\u7f51", "minZoom": 1, "maxZoom": 19, "textColor": "white"});
    ...
    function BaseSearch(loc, opts) {
        ...
        this._opts = {renderOptions: {panel: opts.renderOptions["panel"] || null, map: opts.renderOptions["map"] || null, autoViewport: opts.renderOptions["autoViewport"] || true, selectFirstResult: opts.renderOptions["selectFirstResult"], highlightMode: opts.renderOptions["highlightMode"], enableDragging: opts.renderOptions["enableDragging"] || false}, onSearchComplete: opts["onSearchComplete"] || function () {
        ...
        if (typeof opts != "undefined" && typeof opts["renderOptions"] != "undefined" && typeof opts["renderOptions"]["autoViewport"] != "undefined")this._opts.renderOptions.autoViewport = opts["renderOptions"]["autoViewport"]; else this._opts.renderOptions.autoViewport = true;
        this._opts.renderOptions.panel = baidu.G(this._opts.renderOptions.panel)
    }

    baidu.inherits(BaseSearch, baidu.lang.Class);
    baidu[Textend](BaseSearch[Tprototype], {"getResults": function () {
        if (!this._isMultiKey)return this._results; else return this._arrResults
    ...
    var SearchRequestMgr = {REQ_BASE_URL: "http://api.map.baidu.com/", request: function (cbk, params, userData, path, disableLog) {
        ...
        path = path || "";
        var reqParam;
        if (userData && userData.useEncodeURI)reqParam = jsonToQuery(params, encodeURI); else reqParam = jsonToQuery(params, encodeURIComponent);
        var me = this, url = me.REQ_BASE_URL + path + "?" + reqParam + "&ie=utf-8&oue=1&fromproduct=jsapi";
    ...
    var LocalSearch = function (loc, opts) {
        ...
        if (typeof opts["renderOptions"]["selectFirstResult"] != "undefined" && !opts["renderOptions"]["selectFirstResult"])this.disableFirstResultSelection(); else this.enableFirstResultSelection();
        this._overlays = [];
        this._arrPois = [];
        this._curIndex = -1;
        this._queryList = [];
    ...
    baidu[Textend](LocalSearch[Tprototype], {search: function (keyword, options) {
        this._queryList.push({method: "search", arguments: [keyword, options]})
    }, searchInBounds: function (keyword, bounds, options) {
        this._queryList.push({method: "searchInBounds", arguments: [keyword, bounds, options]})
    }, searchNearby: function (keyword, center, radius, options) {
        this._queryList.push({method: "searchNearby", arguments: [keyword, center, radius, options]})
        ...
        if (typeof cap == "number" && !isNaN(cap))this._opts.pageCapacity = cap < 1 ? LocalSearch.DEFAULT_PAGE_CAPACITY : cap > LocalSearch.MAX_PAGE_CAPACITY ? LocalSearch.DEFAULT_PAGE_CAPACITY : cap; else this._opts.pageCapacity = LocalSearch.DEFAULT_PAGE_CAPACITY
    ...
    exportSymbol(LocalSearchP, {"clearResults": LocalSearchP.clearResults, "setPageCapacity": LocalSearchP.setPageCapacity, "getPageCapacity": LocalSearchP.getPageCapacity, "gotoPage": LocalSearchP.gotoPage, "searchNearby": LocalSearchP.searchNearby, "searchInBounds": LocalSearchP.searchInBounds, "search": LocalSearchP.search, "enableFirstResultSelection": LocalSearchP.enableFirstResultSelection, "disableFirstResultSelection": LocalSearchP.disableFirstResultSelection});
    var BaseRoute = function (loc, opts) {
        BaseSearch.call(this, loc, opts)
    };
    baidu.inherits(BaseRoute, BaseSearch, "BaseRoute");
    baidu[Textend](BaseRoute[Tprototype], {clearResults: function () {
    }});
    ...
    baidu[Textend](TransitRoute[Tprototype], {setPolicy: function (policy) {
        if (policy >= BMAP_TRANSIT_POLICY_LEAST_TIME && policy <= BMAP_TRANSIT_POLICY_AVOID_SUBWAYS)this._opts.policy = policy; else this._opts.policy = BMAP_TRANSIT_POLICY_LEAST_TIME
    }, _internalSearch: function (sn, en) {
        this._queryList.push({method: "_internalSearch", arguments: [sn, en]})
    }, search: function (start, end) {
        this._queryList.push({method: "search", arguments: [start, end]})
        ...
        if (cp >= 1 && cp <= TransitRoute.MAX_PAGE_CAPACITY)this._opts.pageCapacity = TMath.round(cp); else this._opts.pageCapacity = TransitRoute.MAX_PAGE_CAPACITY
    }, toString: function () {
        return"TransitRoute"
    ...
    baidu[Textend](DWRoute[Tprototype], {search: function (start, end, opts) {
        this._queryList.push({method: "search", arguments: [start, end, opts]})
    }});
    ...
    baidu[Textend](DrivingRoute[Tprototype], {setPolicy: function (policy) {
        if (policy >= BMAP_DRIVING_POLICY_LEAST_TIME && policy <= BMAP_DRIVING_POLICY_AVOID_HIGHWAYS)this._opts.policy = policy; else this._opts.policy = BMAP_DRIVING_POLICY_LEAST_TIME
    }});
    ...
    baidu[Textend](Geocoder[Tprototype], {getPoint: function (address, callback, city) {
        this._queryList.push({method: "getPoint", arguments: [address, callback, city]})
    }, getLocation: function (point, callback, opts) {
        this._queryList.push({method: "getLocation", arguments: [point, callback, opts]})
    }, "toString": function () {
        return"Geocoder"
    }});
    var GeocoderP = Geocoder[Tprototype];
    exportSymbol(GeocoderP, {"getPoint": GeocoderP.getPoint, "getLocation": GeocoderP.getLocation});
    ...
    baidu[Textend](Geolocation[Tprototype], {getCurrentPosition: function (callback, opts) {
        this._queryList.push({method: "getCurrentPosition", arguments: [callback, opts]})
    }, getStatus: function () {
        return this._status
    }});
    var GeolocationP = Geolocation[Tprototype];
    exportSymbol(GeolocationP, {"getCurrentPosition": GeolocationP.getCurrentPosition, "getStatus": GeolocationP.getStatus});
    function LocalCity(opts) {
        opts = opts || {};
        opts.renderOptions = opts["renderOptions"] || {};
        this._opts = {renderOptions: {map: opts.renderOptions["map"] || null}};
        this._queryList = [];
        var me = this;
        ModuleMgr.load("othersearch", function () {
            me._asyncSearch()
    ...
    baidu[Textend](LocalCity[Tprototype], {"get": function (callback) {
        this._queryList.push({method: "get", arguments: [callback]})
    }, "toString": function () {
        return"LocalCity"
    }});
    function Boundary() {
    ...
    baidu[Textend](Boundary[Tprototype], {get: function (name, cbk) {
        this._queryList.push({method: "get", arguments: [name, cbk]})
    }, toString: function () {
        return"Boundary"
    }});
    ...
    baidu[Textend](BusLineSearch[Tprototype], {"getBusList": function (lineName) {
        this._queryList.push({method: "getBusList", arguments: [lineName]})
    }, "getBusLine": function (busListItem) {
        this._queryList.push({method: "getBusLine", arguments: [busListItem]})
    ...
    function Autocomplete(opts) {
        ...
        opts = opts || {};
        this._options = {input: opts["input"] || null, baseDom: opts["baseDom"] || null, types: opts["types"] || [], onSearchComplete: opts["onSearchComplete"] || function () {
        }};
    ...
    baidu[Textend](Autocomplete[Tprototype], {_initialize: function () {
    }, show: function () {
        this._show = true
    }, hide: function () {
        this._show = false
    ...
    exportSymbol(window, {"BMap": BMap, "_jsload": _jsload, "BMAP_API_VERSION": BMAP_API_VERSION});
    var MapP = Map[Tprototype];
    exportSymbol(MapP, {"getBounds": MapP.getBounds, "getCenter": MapP.getCenter, "getMapType": MapP.getMapType, "getSize": MapP.getSize, "setSize": MapP.setSize, "getViewport": MapP.getViewport, "getZoom": MapP.getZoom, "centerAndZoom": MapP.centerAndZoom, "panTo": MapP.panTo, "panBy": MapP.panBy, "setCenter": MapP.setCenter, "setCurrentCity": MapP.setCurrentCity, "setMapType": MapP.setMapType, "setViewport": MapP.setViewport, "setZoom": MapP.setZoom, "highResolutionEnabled": MapP.highResolutionEnabled, "zoomTo": MapP.zoomTo, "zoomIn": MapP.zoomIn, "zoomOut": MapP.zoomOut, "addHotspot": MapP.addHotspot, "removeHotspot": MapP.removeHotspot, "clearHotspots": MapP.clearHotspots, "checkResize": MapP.checkResize, "addControl": MapP.addControl, "removeControl": MapP.removeControl, "getContainer": MapP.getContainer, "addContextMenu": MapP.addContextMenu, "removeContextMenu": MapP.removeContextMenu, "addOverlay": MapP.addOverlay, "removeOverlay": MapP.removeOverlay, "clearOverlays": MapP.clearOverlays, "openInfoWindow": MapP.openInfoWindow, "closeInfoWindow": MapP.closeInfoWindow, "pointToOverlayPixel": MapP.pointToOverlayPixel, "overlayPixelToPoint": MapP.overlayPixelToPoint, "getInfoWindow": MapP.getInfoWindow, "getOverlays": MapP.getOverlays, "getPanes": function () {
        return{"floatPane": this._panes.floatPane, "markerMouseTarget": this._panes.markerMouseTarget, "floatShadow": this._panes.floatShadow, "labelPane": this._panes.labelPane, "markerPane": this._panes.markerPane, "markerShadow": this._panes.markerShadow, "mapPane": this._panes.mapPane}
    }, "addTileLayer": MapP.addTileLayer, "removeTileLayer": MapP.removeTileLayer, "pixelToPoint": MapP.pixelToPoint, "pointToPixel": MapP.pointToPixel});
    var MapTypeP = MapType[Tprototype];
    exportSymbol(MapTypeP, {"getTileLayer": MapTypeP.getTileLayer, "getMinZoom": MapTypeP.getMinZoom, "getMaxZoom": MapTypeP.getMaxZoom, "getProjection": MapTypeP.getProjection, "getTextColor": MapTypeP.getTextColor, "getTips": MapTypeP.getTips});
    exportSymbol(window, {"BMAP_NORMAL_MAP": BMAP_NORMAL_MAP, "BMAP_PERSPECTIVE_MAP": BMAP_PERSPECTIVE_MAP, "BMAP_SATELLITE_MAP": BMAP_SATELLITE_MAP, "BMAP_HYBRID_MAP": BMAP_HYBRID_MAP});
    var MercatorProjectionP = MercatorProjection[Tprototype];
    exportSymbol(MercatorProjectionP, {"lngLatToPoint": MercatorProjectionP.lngLatToPoint, "pointToLngLat": MercatorProjectionP.pointToLngLat});
    var PerspectiveProjectionP = PerspectiveProjection[Tprototype];
    exportSymbol(PerspectiveProjectionP, {"lngLatToPoint": PerspectiveProjectionP.lngLatToPoint, "pointToLngLat": PerspectiveProjectionP.pointToLngLat});
    var BoundsP = Bounds[Tprototype];
    exportSymbol(BoundsP, {"equals": BoundsP.equals, "containsPoint": BoundsP.containsPoint, "containsBounds": BoundsP.containsBounds, "intersects": BoundsP.intersects, "extend": BoundsP[Textend], "getCenter": BoundsP.getCenter, "isEmpty": BoundsP.isEmpty, "getSouthWest": BoundsP.getSouthWest, "getNorthEast": BoundsP.getNorthEast, "toSpan": BoundsP.toSpan});
    var OverlayP = Overlay[Tprototype];
    exportSymbol(OverlayP, {"isVisible": OverlayP.isVisible, "show": OverlayP.show, "hide": OverlayP.hide});
    exportProperty(Overlay, "getZIndex", Overlay.getZIndex);
    var OverlayInternalP = OverlayInternal[Tprototype];
    exportSymbol(OverlayInternalP, {"openInfoWindow": OverlayInternalP.openInfoWindow, "closeInfoWindow": OverlayInternalP.closeInfoWindow, "enableMassClear": OverlayInternalP.enableMassClear, "disableMassClear": OverlayInternalP.disableMassClear, "show": OverlayInternalP.show, "hide": OverlayInternalP.hide, "getMap": OverlayInternalP.getMap});
    var MarkerP = Marker[Tprototype];
    exportSymbol(MarkerP, {"setIcon": MarkerP.setIcon, "getIcon": MarkerP.getIcon, "setPosition": MarkerP.setPosition, "getPosition": MarkerP.getPosition, "setOffset": MarkerP.setOffset, "getOffset": MarkerP.getOffset, "getLabel": MarkerP.getLabel, "setLabel": MarkerP.setLabel, "setTitle": MarkerP.setTitle, "setTop": MarkerP.setTop, "enableDragging": MarkerP.enableDragging, "disableDragging": MarkerP.disableDragging, "setZIndex": MarkerP.setZIndex, "getMap": MarkerP.getMap, "addContextMenu": MarkerP.addContextMenu, "removeContextMenu": MarkerP.removeContextMenu, "setAnimation": MarkerP.setAnimation, "setShadow": MarkerP.setShadow, "hide": MarkerP.hide});
    exportSymbol(window, {"BMAP_ANIMATION_DROP": BMAP_ANIMATION_DROP, "BMAP_ANIMATION_BOUNCE": BMAP_ANIMATION_BOUNCE});
    var LabelP = Label[Tprototype];
    exportSymbol(LabelP, {"setStyle": LabelP.setStyle, "setStyles": LabelP.setStyles, "setContent": LabelP.setContent, "setPosition": LabelP.setPosition, "getPosition": LabelP.getPosition, "setOffset": LabelP.setOffset, "getOffset": LabelP.getOffset, "setTitle": LabelP.setTitle, "setZIndex": LabelP.setZIndex, "getMap": LabelP.getMap});
    var IconP = Icon[Tprototype];
    exportSymbol(IconP, {"setImageUrl": IconP.setImageUrl, "setSize": IconP.setSize, "setAnchor": IconP.setAnchor, "setImageOffset": IconP.setImageOffset, "setImageSize": IconP.setImageSize, "setInfoWindowAnchor": IconP.setInfoWindowAnchor, "setPrintImageUrl": IconP.setPrintImageUrl});
    var InfoWindowP = InfoWindow[Tprototype];
    exportSymbol(InfoWindowP, {"redraw": InfoWindowP.redraw, "setTitle": InfoWindowP.setTitle, "setContent": InfoWindowP.setContent, "getContent": InfoWindowP.getContent, "getPosition": InfoWindowP.getPosition, "enableMaximize": InfoWindowP.enableMaximize, "disableMaximize": InfoWindowP.disableMaximize, "isOpen": InfoWindowP.isOpen, "setMaxContent": InfoWindowP.setMaxContent, "maximize": InfoWindowP.maximize, "enableAutoPan": InfoWindowP.enableAutoPan});
    var GraphP = Graph[Tprototype];
    exportSymbol(GraphP, {"getPath": GraphP.getPath, "setPath": GraphP.setPath, "setPositionAt": GraphP.setPositionAt, "getStrokeColor": GraphP.getStrokeColor, "setStrokeWeight": GraphP.setStrokeWeight, "getStrokeWeight": GraphP.getStrokeWeight, "setStrokeOpacity": GraphP.setStrokeOpacity, "getStrokeOpacity": GraphP.getStrokeOpacity, "setFillOpacity": GraphP.setFillOpacity, "getFillOpacity": GraphP.getFillOpacity, "setStrokeStyle": GraphP.setStrokeStyle, "getStrokeStyle": GraphP.getStrokeStyle, "getFillColor": GraphP.getFillColor, "getBounds": GraphP.getBounds, "enableEditing": GraphP.enableEditing, "disableEditing": GraphP.disableEditing});
    var CircleP = Circle[Tprototype];
    exportSymbol(CircleP, {"setCenter": CircleP.setCenter, "getCenter": CircleP.getCenter, "getRadius": CircleP.getRadius, "setRadius": CircleP.setRadius});
    var PolygonP = Polygon[Tprototype];
    exportSymbol(PolygonP, {"getPath": PolygonP.getPath, "setPath": PolygonP.setPath, "setPositionAt": PolygonP.setPositionAt});
    var HotspotP = Hotspot[Tprototype];
    exportSymbol(HotspotP, {"getPosition": HotspotP.getPosition, "setPosition": HotspotP.setPosition, "getText": HotspotP.getText, "setText": HotspotP.setText});
    ...
    exportSymbol(window, {"BMAP_ANCHOR_TOP_LEFT": BMAP_ANCHOR_TOP_LEFT, "BMAP_ANCHOR_TOP_RIGHT": BMAP_ANCHOR_TOP_RIGHT, "BMAP_ANCHOR_BOTTOM_LEFT": BMAP_ANCHOR_BOTTOM_LEFT, "BMAP_ANCHOR_BOTTOM_RIGHT": BMAP_ANCHOR_BOTTOM_RIGHT});
    var ControlP = Control[Tprototype];
    exportSymbol(ControlP, {"setAnchor": ControlP.setAnchor, "getAnchor": ControlP.getAnchor, "setOffset": ControlP.setOffset, "getOffset": ControlP.getOffset, "show": ControlP.show, "hide": ControlP.hide, "isVisible": ControlP.isVisible, "toString": ControlP.toString});
    var NavigationControlP = NavigationControl[Tprototype];
    exportSymbol(NavigationControlP, {"getType": NavigationControlP.getType, "setType": NavigationControlP.setType});
    exportSymbol(window, {"BMAP_NAVIGATION_CONTROL_LARGE": BMAP_NAVIGATION_CONTROL_LARGE, "BMAP_NAVIGATION_CONTROL_SMALL": BMAP_NAVIGATION_CONTROL_SMALL, "BMAP_NAVIGATION_CONTROL_PAN": BMAP_NAVIGATION_CONTROL_PAN, "BMAP_NAVIGATION_CONTROL_ZOOM": BMAP_NAVIGATION_CONTROL_ZOOM});
    var OverviewMapControlP = OverviewMapControl[Tprototype];
    exportSymbol(OverviewMapControlP, {"changeView": OverviewMapControlP.changeView, "setSize": OverviewMapControlP.setSize, "getSize": OverviewMapControlP.getSize});
    var ScaleControlP = ScaleControl[Tprototype];
    exportSymbol(ScaleControlP, {"getUnit": ScaleControlP.getUnit, "setUnit": ScaleControlP.setUnit});
    exportSymbol(window, {"BMAP_UNIT_METRIC": BMAP_UNIT_METRIC, "BMAP_UNIT_IMPERIAL": BMAP_UNIT_IMPERIAL});
    var CopyrightControlP = CopyrightControl[Tprototype];
    exportSymbol(CopyrightControlP, {"addCopyright": CopyrightControlP.addCopyright, "removeCopyright": CopyrightControlP.removeCopyright, "getCopyright": CopyrightControlP.getCopyright, "getCopyrightCollection": CopyrightControlP.getCopyrightCollection});
    exportSymbol(window, {"BMAP_MAPTYPE_CONTROL_HORIZONTAL": BMAP_MAPTYPE_CONTROL_HORIZONTAL, "BMAP_MAPTYPE_CONTROL_DROPDOWN": BMAP_MAPTYPE_CONTROL_DROPDOWN});
    var TileLayerP = TileLayer[Tprototype];
    exportSymbol(TileLayerP, {"getMapType": TileLayerP.getMapType, "getCopyright": TileLayerP.getCopyright, "isTransparentPng": TileLayerP.isTransparentPng});
    var ContextMenuP = ContextMenu[Tprototype];
    exportSymbol(ContextMenuP, {"addItem": ContextMenuP.addItem, "addSeparator": ContextMenuP.addSeparator, "removeSeparator": ContextMenuP.removeSeparator});
    var MenuItemP = MenuItem[Tprototype];
    exportSymbol(MenuItemP, {"setText": MenuItemP.setText});
    ...
    exportSymbol(window, {"BMAP_STATUS_SUCCESS": BMAP_STATUS_SUCCESS, "BMAP_STATUS_CITY_LIST": BMAP_STATUS_CITY_LIST, "BMAP_STATUS_UNKNOWN_LOCATION": BMAP_STATUS_UNKNOWN_LOCATION, "BMAP_STATUS_UNKNOWN_ROUTE": BMAP_STATUS_UNKNOWN_ROUTE, "BMAP_STATUS_INVALID_KEY": BMAP_STATUS_INVALID_KEY, "BMAP_STATUS_INVALID_REQUEST": BMAP_STATUS_INVALID_REQUEST, "BMAP_STATUS_PERMISSION_DENIED": BMAP_STATUS_PERMISSION_DENIED, "BMAP_STATUS_SERVICE_UNAVAILABLE": BMAP_STATUS_SERVICE_UNAVAILABLE, "BMAP_STATUS_TIMEOUT": BMAP_STATUS_TIMEOUT});
    exportSymbol(window, {"BMAP_POI_TYPE_NORMAL": BMAP_POI_TYPE_NORMAL, "BMAP_POI_TYPE_BUSSTOP": BMAP_POI_TYPE_BUSSTOP, "BMAP_POI_TYPE_BUSLINE": BMAP_POI_TYPE_BUSLINE, "BMAP_POI_TYPE_SUBSTOP": BMAP_POI_TYPE_SUBSTOP, "BMAP_POI_TYPE_SUBLINE": BMAP_POI_TYPE_SUBLINE});
    exportSymbol(window, {"BMAP_TRANSIT_POLICY_LEAST_TIME": BMAP_TRANSIT_POLICY_LEAST_TIME, "BMAP_TRANSIT_POLICY_LEAST_TRANSFER": BMAP_TRANSIT_POLICY_LEAST_TRANSFER, "BMAP_TRANSIT_POLICY_LEAST_WALKING": BMAP_TRANSIT_POLICY_LEAST_WALKING, "BMAP_TRANSIT_POLICY_AVOID_SUBWAYS": BMAP_TRANSIT_POLICY_AVOID_SUBWAYS, "BMAP_LINE_TYPE_BUS": BMAP_LINE_TYPE_BUS, "BMAP_LINE_TYPE_SUBWAY": BMAP_LINE_TYPE_SUBWAY, "BMAP_LINE_TYPE_FERRY": BMAP_LINE_TYPE_FERRY});
    var BaseRouteP = BaseRoute[Tprototype];
    exportSymbol(BaseRouteP, {"clearResults": BaseRouteP.clearResults});
    var TransitRouteP = TransitRoute[Tprototype];
    exportSymbol(TransitRouteP, {"setPolicy": TransitRouteP.setPolicy, "toString": TransitRouteP.toString, "setPageCapacity": TransitRouteP.setPageCapacity});
    exportSymbol(window, {"BMAP_DRIVING_POLICY_LEAST_TIME": BMAP_DRIVING_POLICY_LEAST_TIME, "BMAP_DRIVING_POLICY_LEAST_DISTANCE": BMAP_DRIVING_POLICY_LEAST_DISTANCE, "BMAP_DRIVING_POLICY_AVOID_HIGHWAYS": BMAP_DRIVING_POLICY_AVOID_HIGHWAYS});
    exportSymbol(window, {"BMAP_HIGHLIGHT_STEP": BMAP_HIGHLIGHT_STEP, "BMAP_HIGHLIGHT_ROUTE": BMAP_HIGHLIGHT_ROUTE});
    exportSymbol(window, {"BMAP_ROUTE_TYPE_DRIVING": BMAP_ROUTE_TYPE_DRIVING, "BMAP_ROUTE_TYPE_WALKING": BMAP_ROUTE_TYPE_WALKING});
    exportSymbol(window, {"BMAP_ROUTE_STATUS_NORMAL": BMAP_ROUTE_STATUS_NORMAL, "BMAP_ROUTE_STATUS_EMPTY": BMAP_ROUTE_STATUS_EMPTY, "BMAP_ROUTE_STATUS_ADDRESS": BMAP_ROUTE_STATUS_ADDRESS});
    var DrivingRouteP = DrivingRoute[Tprototype];
    exportSymbol(DrivingRouteP, {"setPolicy": DrivingRouteP.setPolicy});
    var AutocompleteP = Autocomplete[Tprototype];
    exportSymbol(AutocompleteP, {"show": AutocompleteP.show, "hide": AutocompleteP.hide, "setTypes": AutocompleteP.setTypes, "setLocation": AutocompleteP.setLocation, "search": AutocompleteP.search, "setInputValue": AutocompleteP.setInputValue});
    var BoundaryP = Boundary[Tprototype];
    exportSymbol(BoundaryP, {"get": BoundaryP.get});
    BMap.apiLoad();
})()