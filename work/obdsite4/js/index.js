...
$(function(){
	...
	function loadIframe(bc){
		if(client.width<=720 && !loadedMobile){
			$('<iframe id="iframeMobile" src="index_mobile.html" style="display:none"></iframe>').appendTo('body').on('load',function(e){
				...
				aBlock = [$('#animation02'),$('#animation03'),$('#animation04'),$('#animation05'),$('#animation06'),$('#animation07'),$('#animation08'),$('#animation09'),$('#animation10'),$('#animation11')];
	...
	function eachBlock(block,i){
		if(scrollTop > block.offset().top-300 && scrollTop < block.offset().top+500){
			...
			if(i == aBlock.length-1){
				$('#footer').addClass('ani01Start');
			}
			if(i == 7){
				console.log('aaa')
				setTimeout(function(){
